...
void main()
{
    ...
    float fractalWhiteMotion = cos(x * frequency);
    //
    //
    ...
    fractalWhiteMotion += amplitude * noise(frequency*x);    
	frequency *= lacunarity;
	amplitude *= gain;
	}
    
    // Y OFFSET
    fractalBrownMotion = (fractalBrownMotion + 1.) / 4.0;
    fractalWhiteMotion = (fractalWhiteMotion + 1.) / 4.0;


    // Create a smooth transition for alpha based on the y-coordinate's proximity to fractalBrownMotion
    // This is used to draw a line that smoothly fades in and out as it approaches or moves away from fractalBrownMotion
    float alpha = smoothstep(
        0.002,
        0.010, // Edge1: Upper edge of the transition range
        abs(uv.y - fractalBrownMotion) // Value: The distance between the current y-coordinate and fractalBrownMotion
    );

    float alphaTwo = smoothstep(
    	0.002,
        0.010,
        abs(uv.y - fractalWhiteMotion)
    );
    ...
    col *= fractalWhiteMotion * alphaTwo;
    vec3 waveColor = vec3(uv.x, uv.x * uv.y, uv.y);
    vec3 backgroundColor = vec3(.0); // White background
    

    
	vec3 colOut = mix(waveColor, backgroundColor, alpha * alphaTwo);
